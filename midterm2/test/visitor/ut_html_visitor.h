...
TEST_F(HtmlVisitorTest, VisitorComplexParagraphIsCorrect){
...
TEST_F(HtmlVisitorTest, VisitorEmptyParagraphIsCorrect){
    Article* empty_paragraph = new Paragraph(1, "this is a empty paragraph");
    ArticleVisitor* hv = new HtmlVisitor();
    empty_paragraph->accept(hv);
    ASSERT_EQ(hv->getResult(), "<div><h1>this is a empty paragraph</h1></div>");

    delete empty_paragraph;
    delete hv;
}

TEST_F(HtmlVisitorTest, VisitorSimpleParagraphIsCorrect){
    Article* p1 = new Paragraph(1, "p1");
    p1->add(new ListItem("list1"));
    p1->add(new Text("text1"));

    std::string test = "<div><h1>p1</h1><li>list1</li><span>text1</span></div>";

    ArticleVisitor* hv = new HtmlVisitor();
    p1->accept(hv);
    ASSERT_EQ(hv->getResult(), test);
